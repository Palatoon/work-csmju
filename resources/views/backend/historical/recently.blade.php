...
<div class="d-sm-flex align-items-center mb-4 row">
    ...
    <div class="col-md-5">
        <form method="post" id='ddfilter' enctype="multipart/form-data" action="{{ action('Backend\HistoricalController@recently') }}">
            @csrf
...
<script type="text/javascript">
    ...
    {
        var d = new Date();
        d = d.setHours(d.getHours() - i);
        t.push(moment(new Date(d)).format("HH:00:00"));
    }
    var len=t.length;
    for(var i =0 ;i<len;i++)
    {
        ...
        for (var i = 0; i < IoTData.length; i++) {
            switch (IoTData[i].DataCode) {
                ...
                case "ELECU":
                    ...
                    var d = new Date(IoTData[i].Timestamp);
                    time.push(d.getTime());
                    break;
                default:
                    ;
    ...
    var handleAreaChart = function() {
            ...
            xaxis: {
                ...
                labels: {
                    ...
                    formatter: function(value) {
                        //console.log(moment(new Date(value)).format("HH:mm:ss"));
        ...
        setInterval(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('recently.data') }}",
                data: {
                    sltSenSor: $('#sltSenSor').val()
                },
                success: function(data) {
                    ...
                    if (data) {
                        for (var i = 0; i < data.length; i++) {
                            switch (data[i].DataCode) {
                                case "ELECV1":
                                    v1.push(data[i].Value);
                                    break;
                                case "ELECV2":
                                    v2.push(data[i].Value);
                                    break;
                                case "ELECV3":
                                    v3.push(data[i].Value);
                                    break;
                                case "ELECI1":
                                    I1.push(data[i].Value);
                                    break;
                                case "ELECI2":
                                    I2.push(data[i].Value);
                                    break;
                                case "ELECI3":
                                    I3.push(data[i].Value);
                                    break;
                                case "ELECP1":
                                    P1.push(data[i].Value);
                                    break;
                                case "ELECP2":
                                    P2.push(data[i].Value);
                                    break;
                                case "ELECP3":
                                    P3.push(data[i].Value);
                                    break;
                                case "ELECU":
                                    U.push(data[i].Value);
