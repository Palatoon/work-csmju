...
<div class="d-sm-flex align-items-center mb-4 row">
    ...
    <div class="col-md-5">
        <form method="post" id='ddfilter' enctype="multipart/form-data" action="{{ action('Backend\HistoricalController@day') }}">
            @csrf
...
<script type="text/javascript">
    ...
    /*for(var i=0 ;i<=14;i++)
    {
        ...
        t.push(moment(new Date(d)).format("DD/MM"));
    }
    var len=t.length;
    ...
    var U = [];
    if (IoTData) {
        if (IoTData.length < 140) {
            for (var i = IoTData.length; i <= 140; i += 10) {
                v1.push(0);
                v2.push(0);
                v3.push(0);
                I1.push(0);
        ...
        for (var i = 0; i < IoTData.length; i++) {
            switch (IoTData[i].DataCode) {
                ...
                case "ELECU":
                    ...
                    time.push(moment(new Date(d)).format("DD/MM"));
                    break;
                default:
                    ;
                    break;
