...
<div class="panel panel-inverse">
    ...
    <div class="panel-body panel-form">
        <form method="POST" action="{{ route('group.store') }}" class="form-horizontal form-bordered" id="create-form">
            @csrf
            <div class="form-group">
                ...
                <input class="form-control" type="hidden" id="sid" name="sid" required readonly />

                <div class="pb-1">
                    <label class="font-weight-bold" for="name">Group Name</label>
                    <input class="form-control" type="text" id="name" name="name" placeholder="Group Name" required />
                </div>
                ...
                <div class="pb-1">
                    ...
                    <input class="form-control" type="number" id="hours" name="hours" min="0" required placeholder="Hours/Week">
                </div>
    ...
    <script>
        ...
        $('#search_group').click(function() {
            ...
            if (group.length >= 2) {
                ...
                $.ajax({
                    url: "http://ws.lanna.co.th/Ads/searchgroup",
                        ...
                        if (json.length > 0) {
                            $.each(json, function(index, data) {
                                ...
                                li = li + '<li class="list-group-item"><div class="d-flex flex-row justify-content-between align-items-center">' +
                                    ...
                                    'onclick="addgroup(\'' + data.ID + '\',\'' + data.Name + '\',\'' + data.Address + '\')">Add Group</button></div></div ></li>';
                                $('.listgroup').append(li);
                            });
                        } else {
        ...
        function addgroup(sid, name, disting) {
            $('.list-group').html('');
            $('#search_group').html('ค้นหา');
            $('#sid').val(sid);
            $('#name').val(name);
