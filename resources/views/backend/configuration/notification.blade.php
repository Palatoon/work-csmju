				...
				<table class="table table-striped table-td-valign-middle">
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $item->notify_status == 1 ? 'True' : 'False' }}</td>
							<td>
								...
								@else
								<button class="btn btn-xs btn-info" onclick="btn_edit_notify({{$item->id}},'{{$item->name_th}}','{{$item->value}}','{{$item->unit}}','{{$item->notify_status}}');"> Edit</button>
								@endif
